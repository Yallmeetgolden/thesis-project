...
$thresholdInput = $_POST['threshold'] ?? '0.92';
$threshold = (float)$thresholdInput;
if ($threshold < 0.70 || $threshold > 0.999) {
    $threshold = 0.92;
}
...
$scriptPath = realpath(__DIR__ . '/../../python_service/duplicate_pose_analyzer.py');
if (!$scriptPath || !file_exists($scriptPath)) {
    fail(500, 'Duplicate pose analyzer script not found');
}
...
if (!is_resource($process)) {
    fail(500, 'Failed to launch duplicate analyzer process');
}
...
if ($exitCode !== 0) {
    ...
    fail(502, 'Duplicate analysis failed. ' . ($detail !== '' ? $detail : 'No error detail from process'));
}

$data = json_decode($stdout ?: '', true);
if (!is_array($data)) {
    fail(502, 'Duplicate analyzer returned invalid JSON');
}

if (!($data['success'] ?? false)) {
    $msg = $data['error'] ?? 'Unknown duplicate analysis error';
    fail(502, $msg);
}
